...
namespace unit\Drivers\Pgsql\Builder\Builders;
...
class UpdateBuilderTest extends BuilderTestCase
{
    ...
    public function testLimitAndOffsetAreNotBuiltForPostgres()
    {
        // PostgreSQL does not support LIMIT/OFFSET on an UPDATE statement,
        // so they must be ignored even when set on the query.
        $this->assertQueryResults([
            [Update::create()->to('table')->set(['a' => 1])->offset(10)->limit(5), 'UPDATE "table" SET "a" = :t0', [
                ':t0' => [1, null]
            ]],
        ]);
    }
