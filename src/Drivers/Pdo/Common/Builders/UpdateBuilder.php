...
use ArekX\PQL\Drivers\Pdo\Common\Builders\Traits\QuoteNameTrait;
use ArekX\PQL\Drivers\Pdo\Common\Builders\Traits\SubQueryTrait;
use ArekX\PQL\Drivers\Pdo\Common\Builders\Traits\WhereTrait;
use ArekX\PQL\Drivers\Pdo\Common\Builders\Traits\WrapValueTrait;
...
class UpdateBuilder extends QueryPartBuilder
{
    ...
    protected function getPartBuilders(): array
    {
        return [
            ...
            'limit' => fn($parts, $state) => $state->supportsModifyLimit() ? $this->buildLimit($parts) : null,
            'offset' => fn($parts, $state) => $state->supportsModifyLimit() ? $this->buildOffset($parts) : null,
        ];
    }
     ...
     * @return string
     */
    protected function buildTo(StructuredQuery|string $parts, CommonQueryBuilderState $state): string
    {
        if ($parts instanceof StructuredQuery) {
            return $this->buildQuery($parts, $state);
        }

        return $this->quoteName($parts, $state);
    }
     ...
     * @return string
     */
    protected function buildSet(StructuredQuery|array $parts, CommonQueryBuilderState $state): string
    {
        if ($parts instanceof StructuredQuery) {
        ...
        foreach ($parts as $key => $value) {
            $results[] = $this->quoteName($key, $state) . ' = ' . $this->buildWrapValue($value, $state);
        }
