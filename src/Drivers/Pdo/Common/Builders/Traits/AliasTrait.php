...
trait AliasTrait
{
     ...
     * @param CommonQueryBuilderState $state State of the builder
     * @return string
     * @see SubQueryTrait::buildSubQuery()
     * @see QuoteNameTrait::quoteName()
     */
    protected function buildAliasedNames(StructuredQuery|array|string $namePart, CommonQueryBuilderState $state): string
    {
        ...
        if (is_string($namePart)) {
            return $this->quoteName($namePart, $state);
        }
        ...
        foreach ($namePart as $as => $item) {
            if ($item instanceof StructuredQuery) {
                ...
                $item = $this->quoteName($item, $state);
            }


            if (is_string($as)) {
                $item .= " AS " . $this->quoteName($as, $state);
            }
