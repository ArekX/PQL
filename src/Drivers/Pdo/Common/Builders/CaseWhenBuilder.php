...
class CaseWhenBuilder extends QueryPartBuilder
{
    ...
    {
        return [
            'when' => fn($name, CommonQueryBuilderState $state) => $this->buildWhen($name, $state),
            'else' => fn($name, CommonQueryBuilderState $state) => $this->buildElse($name, $state),
        ];
    }
     ...
     * @param CommonQueryBuilderState $state
     * @return string
     * @throws Exception
     */
    protected function buildWhen(array $whenList, CommonQueryBuilderState $state): string
    {
        $result = [];
     ...
     * @throws Exception
     */
    protected function buildElse(StructuredQuery|array $else, CommonQueryBuilderState $state): string
    {
